...
if(!kanal) return message.channel.send(`Jail log kanalı ayarlı değil.`)
  ...
  if(kişi.hasPermission("MANAGE_GUILD")) return message.channel.send(`Bu kişiyi susturamam.`)
...
if(!sebep) sebep = 'Sebep belirtilmemiş.'
  ...
  .setColor(`#f3c7e1`)
  .setDescription(`Bir mahkum daha hapishaneye yollandı.`)
  ...
  .setAuthor(message.author.tag, message.author.avatarURL)
  .setDescription(`Birisi cezalıdan çıktı!`)
  .addField(`**Çıkan olan:**`, kişi, true)
...
db.set(`${message.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
    kanal.send(wasted)
    message.channel.send(`${kişi} isimli kişi başarıyla cezalı kısmına gönderildi.`)
    setTimeout(async () =>{
...
exports.help = {
 ...
 kategori: 'Moderasyon',
 permLvl: 'Bulunmuyor, (!jail-yetkilisi ayarla)'
};